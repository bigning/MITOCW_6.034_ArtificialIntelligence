...
def focused_evaluate(board):
    ...
    if board.is_game_over():
        ...
        score = (board.longest_chain(board.get_current_player_id()) ** 2) * 10
...
def alphabeta_find_board_value(original_player, minmax_flag, board, depth, eval_fn,
                             get_next_moves_fn,
                             is_terminal_fn, already_val):
    """
    ...
    if is_terminal_fn(depth, board):
        score = eval_fn(board)
        if not isinstance(board, ConnectFourBoard):
            if minmax_flag == -1:
                return -1*score
            return score
        if board.get_current_player_id() == original_player:
            return score
        else:
            return -1.0 * score
    ...
    best_myval = 0
    if minmax_flag == 1:
        best_myval = NEG_INFINITY
    else:
        best_myval = INFINITY
    for move, new_board in get_next_moves_fn(board):
        val = alphabeta_find_board_value(original_player, minmax_flag,new_board, depth-1, eval_fn, get_next_moves_fn, is_terminal_fn, best_myval)
        if best_val == None or (minmax_flag == 1 and val > best_val) or (minmax_flag == -1 and val < best_val):
            best_val = val
            best_myval = val
        if minmax_flag == 1:
            if val > already_val:
...
def alpha_beta_search(board, depth,
    ...
    if isinstance(board, ConnectFourBoard): 
        original_player = board.get_current_player_id()
    previous_max = NEG_INFINITY
    for move, new_board in get_next_moves_fn(board):
        val =  alphabeta_find_board_value(original_player, minmax_flag, new_board, depth-1, eval_fn,
                                            get_next_moves_fn,
                                            is_terminal_fn, previous_max)
...
def better_evaluate(board):
    if board.is_game_over():
        ...
        score = (board.longest_chain(board.get_current_player_id()) ** 2) * 10
        #score -= (board.longest_chain(board.get_other_player_id()) ** 2) * 10
        if board.longest_chain(board.get_current_player_id()) >= 4:
            return 10000
        if board.longest_chain(board.get_other_player_id()) >= 4:
            return -10000
        # Prefer having your pieces in the center of the board.
        for row in range(6):
            for col in range(7):
...
WHAT_I_FOUND_INTERESTING = "s"
WHAT_I_FOUND_BORING = "s"
NAME = "Ning Wang"
EMAIL = "nw932@nyu.edu"
