...
def most_misclassified(classifier, n=5):
    ...
    """
    index = [i[0] for i in sorted(enumerate(classifier.data_weights), key=lambda x:x[1], reverse=True)]
    res = []
    for i in range(n):
        res.append(legislator_info(classifier.data[index[i]]))
    return res
...
#classifiers_for_best_ensemble = ['svmr', 'nb','svmr', 'knn','svmp3', 'svmr', 'svms', 'svml', 'maj']
classifiers_for_best_ensemble = ['svmp3', 'svml', 'svms', 'svmr']
