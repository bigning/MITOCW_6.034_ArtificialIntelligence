...
same_identify_rule = IF(OR('male (?x)', 'female (?x)'), THEN('same-identify (?x) (?x)'))
brother_rule = IF(AND('male (?x)', 'parent (?z) (?x)', 'parent (?z) (?y)', NOT('same-identify (?x) (?y)')), THEN('brother (?x) (?y)'))
sister_rule = IF(AND('female (?x)', 'parent (?z) (?x)', 'parent (?z) (?y)', NOT('same-identify (?x) (?y)')), THEN('sister (?x) (?y)'))
mother_fule = IF(AND('female (?x)', 'parent (?x) (?y)'), THEN('mother (?x) (?y)'))
father_fule = IF(AND('male (?x)', 'parent (?x) (?y)'), THEN('father (?x) (?y)'))
son_rule = IF(AND('male (?x)', 'parent (?y) (?x)'), THEN('son (?x) (?y)'))
daughter_rule = IF(AND('female (?x)', 'parent (?y) (?x)'), THEN('daughter (?x) (?y)'))
cousin_rule = IF(AND('parent (?u) (?x)', 'parent (?v) (?y)', OR('sister (?u) (?v)', 'brother (?u) (?v)')), THEN('cousin (?x) (?y)'))
grand_parent_rule = IF(AND('parent (?x) (?z)', 'parent (?z) (?y)'), THEN('grandparent (?x) (?y)'))
grandchild_rule = IF(AND('grandparent (?y) (?x)'), THEN('grandchild (?x) (?y)'))


family_rules = [same_identify_rule, brother_rule, sister_rule, mother_fule, father_fule, son_rule, daughter_rule, cousin_rule, grand_parent_rule, grandchild_rule]                    # fill me in
...
WHAT_I_FOUND_INTERESTING = 'last'
WHAT_I_FOUND_BORING = 'begin'
